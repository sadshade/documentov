(function ($) {
  var default_options = {
    ...
    templateFile: '<div id="fileup-[INPUT_ID]-[FILE_NUM]" class="fileup-file [TYPE]">' +
      ' <input type="hidden" name="[FIELD_NAME][]" id="input-file-[FIELD_ID]_[FILE_NUM]" value=""/>' +
      ...
      switch (event) {
        ...
        case 'error_load':
          ...
          $file.find('.fileup-result')
            ...
            .text(options.i18n.error + ': ' + response);
          break;
        case 'old_browser':
          alert(options.i18n.errorOldBrowser);
          break;
      }
    },
    ...
    onSuccessSystem: function (response, file_number, file) {
      ...
      $file.find('.fileup-result')
        ...
        .text(options.i18n.complete);
      var resp = JSON.parse(response);
      $('#input-file-' + options.field_id + "_" + file_number).val(resp['success']);
    },
  ...
  function init(param1) {
    ...
    if (input && input.type === 'file') {
      ...
      events.addEvent(input, 'afterRemove', options.onAfterRemove);
      ...
      if (options.files.length > 0) {
        for (var i = 0; i < options.files.length; i++) {
          ...
          tpl = tpl.replace(/\[FIELD_NAME\]/g, options.field_name);
          tpl = tpl.replace(/\[FIELD_ID\]/g, options.field_id);
          ...
          tpl = tpl.replace(/\[UPLOAD\]/g, options.i18n.upload);
          tpl = tpl.replace(/\[REMOVE\]/g, options.i18n.remove);
          tpl = tpl.replace(/\[ABORT\]/g, options.i18n.abort);
  ...
  function dropFiles(event) {
    ...
    if (files.length) {
      for (var i = 0; i < files.length; i++) {
        ...
        if (options.filesLimit > 0 && $('input[name*=\'' + options.field_name + '\']').length >= options.filesLimit) {
  ...
  function appendFiles() {
    ...
    if (this.files.length) {
      ...
      var count_upload_files = $('input[name*=\'' + options.field_name + '\']').length;
      if ($('input[name*=\'' + options.field_name + '\']').length === 1 && $('input[name*=\'' + options.field_name + '\']').val() === "") {
      ...
      for (var i = 0; i < this.files.length; i++) {
        ...
        if (options.filesLimit > 0 && (count_upload_files + i) >= options.filesLimit) {

          if ($('input[name*=\'' + options.field_name + '\']').val()) {
  ...
  function uploadFile(input, fileContainer) {
    ...
    xhr.onload = function () {
      ...
      try {
        ...
        resp = false;
        let textError = options.i18n['error' + xhr.status] || xhr.statusText;
        events.callEvent(input, 'error', ['bad_status', file, file_number, textError]);
      }
      if (resp) {
        if (xhr.status === 200 && resp['success']) {
          events.callEvent(input, 'success', [xhr.responseText, file_number, file]);
        } else {
          events.callEvent(input, 'error', ['bad_status', file, file_number, resp['error']]);
        }
      }
      ...
      if (input && input.type === 'file' && typeof input.fileup === 'object') {
        ...
        if (file_number === '*') {
          ...
          $('[id^=fileup-' + inputID + '-]').fadeOut('fast', function () {
            $(this).remove();
            events.callEvent(input, 'afterRemove', []);
          });
