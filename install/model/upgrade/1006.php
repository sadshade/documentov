...
class ModelUpgrade1006 extends Model {
	public function upgrade() {
		...
		if (is_file(DIR_DOCUMENTOV . 'config.php')) {
			$files = glob(DIR_DOCUMENTOV . '{config.php,admin/config.php}', GLOB_BRACE);
		...
		if (file_exists(DIR_DOCUMENTOV . 'download')) {
			$this->recursive_move(DIR_DOCUMENTOV . 'download', DIR_SYSTEM . 'storage/download');
		}
